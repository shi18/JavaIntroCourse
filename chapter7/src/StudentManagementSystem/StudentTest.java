...
public class StudentTest {
    ...
    private static void recallPassword(ArrayList<User> userList, Scanner sc) {
        ...
        while (true) {
            ...
            int userIndex = checkPhoneNum(inputPhoneNum, userList);
            if (userIndex == -1) {
            ...
            boolean passSecQuestCheckFlag = checkSecQuest(userIndex, firstAns, secAns, userList);
...
//                    System.exit(0);
    ...
    public static boolean checkUsernameLegality(String inputUsername) {
        ...
        if (len > 12 || len < 5) {
            ...
            // username check
            for (int i = 0; i < len; i++) {
                char tmpC = inputUsername.charAt(i);
    ...
    public static boolean checkPassLegality(String firstInputPass) {
        ...
        for (int i = 0; i < firstInputPass.length(); i++) {
            char c = firstInputPass.charAt(i);
            if (c >= 'a' && c <= 'z') {
                lowerCaseCount++;
            }
            if (c >= 'A' && c <= 'Z') {
    ...
    public static boolean checkSecQuest(int userIndex, String firstAns, String secAns, ArrayList<User> userList) {
        if (userList.get(userIndex).getSecureQuestion()[1][0].equals(firstAns)
                && userList.get(userIndex).getSecureQuestion()[1][1].equals(secAns)) {
            return true;
        }
        return false;
    }

    public static int checkPhoneNum(String inputPhoneNum, ArrayList<User> userList) {
        for (int i = 0; i < userList.size(); i++) {
            if (userList.get(i).getPhoneNumber().equals(inputPhoneNum)) {
                return i;
            }
        }
        return -1;
